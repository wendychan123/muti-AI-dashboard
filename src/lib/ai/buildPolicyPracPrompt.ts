...
⚠️ 不得假設學校個別情境。
⚠️ 不得給學生個別學習建議。
⚠️ 針對需關注的單位名稱、學校代碼、關鍵重點行動建議，請務必使用 **粗體** 標示，系統將自動套用視覺效果！（例如：**A國小**的表現落後，建議**導入數位教具**）
...
• （針對該圖表提出 1 點解析）
  ...
  ◦ 說明數據呈現出的結構現象，並點名異常的 **學校代碼**
  ◦ 說明可能的制度層面影響（不得超出數據）
`
    : `
• （列出 1 點跨圖整合發現）
  ◦ 每一點必須同時引用至少 2 張圖的【分析焦點】
  ◦ 必須說明圖與圖之間的因果或結構關聯，並點名落後的 **學校代碼**
  ◦ 不得僅做單圖描述
`
}
...
潛在風險：
  ◦ 指出 1 項需監測之結構性風險 （如：校際差距擴大、特定 **學校代碼** 嚴重落後）
  ◦ 說明建議監測之指標或警戒訊號
