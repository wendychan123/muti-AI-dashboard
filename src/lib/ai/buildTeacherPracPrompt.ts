...
const CHART_INTERPRETATION_GUIDES: Record<TeacherPracChartTarget, string> = {
  teacher_overview: 
    "【總覽練習表現】：請從整體參與率、精熟率與人均練習量，評估全校/該年級在該科目的整體健康度。若精熟率低但練習量高，代表存在系統性的學習瓶頸；若參與率極低，代表教學進度可能尚未跟上。",
  diagnostic: 
    "【教學診斷指標(四象限)】：X軸為「參與率(%)」，Y軸為「平均正確率(%)」。請直接點出落在「瓶頸區 (高參與、低正確率)」的具體知識節點名稱，這代表多數學生都已進行練習卻普遍卡關，需優先進行全班性補救教學。落在「低參與」區則表示該單元多數學生尚未作答，樣本數不足以代表全班成效。",
  practice_trend: 
  ...
  //indicator_treemap: 知識節點熱力圖】：區塊面積代表「參與練習人數」，顏色深淺代表「平均精熟率」。請務必優先抓出「大面積且顏色偏白或極淺紫」的區塊，這代表多數學生都有練習但普遍未達標的「全校性教學痛點」。"
  student_risk: 
...
你你的目標是提供「極度聚焦、具體點名、可立即操作」的實質教學指引，拒絕任何空泛的場面話（如：請多加關心學生、請加強複習）。
請使用「專業、具同理心、實務導向」的語氣，，不得使用口語化表達或對話式語句，且避免過度僵硬的行政用語。

⚠️ 所有判斷必須嚴格依據提供的數據。
⚠️ 必須嚴格依據 JSON 數據，直接寫出「出問題的單元名稱」或「學生的 ID」。
⚠️ 針對重點行動建議等關鍵方法，請務必使用 **粗體** 包覆標示！系統會自動將其變色高亮（例如：在 **分數的加減** 單元中...）。
⚠️ 不得假設學校個別情境。
⚠️ 不得給學生個別學習建議。
⚠️ 所有行動建議必須具備教學現場的實務操作性，並嚴格引用下方提供的【教學知識庫】。
...
以下是系統針對該科目/節點檢索出的「專屬教學知識庫」。給予教學行動建議時，絕對必須引用這裡的【教師策略(Teacher)】或【補救路徑】來提出解方，避免給出空泛的「多加複習」等建議：
...
- 請進行「深入解析」。
- 分析該數據背後隱藏的教學意義。
- 識別出該維度下的優勢學生群或弱點知識節點（如：正確率最低的單元、風險值最高的學生ID）。
- 說明這對接下來的教學進度有何影響。`
    : `
...
- 第二句：直接點名從圖表數據中抓出最核心的「最弱單元名稱」或「待輔助的學生ID」。
...
${
  isSingle
    ? `
  •（針對該圖表提出 1 點解析）
  ◦ 描述數據分布特徵，明確點出「需要補救的知識節點名稱」或「學生 ID」，說明其落差或異常趨勢。`
  : `
•（提出 1 點跨圖整合發現）
  ◦ 每一點必須結合至少 2 張圖表的資訊（例如：參與度下降是否導致了正確率走勢下滑）
  ◦ 必須說明圖與圖之間的因果或結構關聯，不得僅做單圖描述
  ◦ 點名哪些單元是「全班共同卡關」，哪些是「少數學生落後」`
}

｜教學提醒
優勢：
  ◦ 指出 1 個表現達標或穩定的單元名稱（若無，請寫「當前數據偏弱，需全面檢視教學策略」）。

風險：
  ◦ 明確點出當前「最需立即輔助干預的單元名稱」或「未精熟學生ID」，說明若不處理，將如何影響後續銜接。
